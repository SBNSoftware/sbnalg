...
An example of a interactive session counting the number of muons from `generator`:
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
import galleryUtils, ROOT
sampleEvents = galleryUtils.makeEvent("prodcorsika_genie_standard_icarus_20250314T220317-GenGenieCorsika.root")
TruthTag = ROOT.art.InputTag("generator")

for event in galleryUtils.EventIterator(sampleEvents):
  truths = event.getProduct[ROOT.std.vector[ROOT.simb.MCTruth]](TruthTag)
  
  nMuons = sum(
    abs(truth.GetParticle(i).PdgCode()) == 13
    for truth in truths for i in range(truth.NParticles())
    )
  print(f"{event.eventAuxiliary().id()}: {nMuons} muons")
  
# for all events
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
For more control on the loop, see `galleryUtils.forEach()`.
...
class HandleMaker:
  ...
  def validHandle(self,
    ...
    raise DeprecationWarning("""
      HandleMaker and make_getValidHandle() are deprecated as gallery and cppyy
      now support the equivalent functionality with an easier syntax; e.g.
      
          data = event.getProduct[ROOT.std.vector[int]](tag)
...
class EventIterator:
  """
  Forward iterator for an event sequence.
